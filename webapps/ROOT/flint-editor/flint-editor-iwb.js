...
$(document)
		.ready(
				function($) {
					...
					// Copied from markitup to align with jQuery 1.9 
					// fix when the new version of Flint becomes available
					...
					$.browser = browser;
// end of jQuery 1.9 alignment patch
					
					
					var sampleFluidopsQuery1 = "SELECT ?org (COUNT(?project) AS ?countProjects) WHERE { \n" 
						...
						"endpoints" : [
								{
									...
									"uri" : contextPath + "/sparql",
									"modes" : ["sparql11query"],
									queries : [
											{
												"name" : "Sample Testbed query",
												"description" : "Test fluidOps query.",
												"query" : sampleFluidopsQuery1
											} ]
								} ],
						"defaultModes" : [ {
							"name" : "SPARQL 1.0",
					...
					if(typeof(iwbNamespacePrefixes)!= "undefined") {
						flintConfig.namespaces = iwbNamespacePrefixes;
					}
			...
			if (activeDataItem) {
				if (activeDataItem.prefixes != null) {
					try {
						...
						var i;
						for (i = 0; i < activeDataItem.prefixes.length; i++) {
							listText += "<tr><td class='flint-prefix'>"
...
//						listText = "<ul>" + listText + "</ul>";
		...
		function displayProperties() {
			$('#flint-sidebar-content').text("");
			if(typeof(iwbProperties!="undefined")) {
				try {
					if(iwbProperties.length>0) {
						var listText = "";
						var i;
						for (i = 0; i < iwbProperties.length; i++) {
		...
		function displayClasses() {
			$('#flint-sidebar-content').text("");
			if(typeof(iwbClasses!="undefined")) {
				try {
					if(iwbClasses.length>0) {
						var listText = "";
						var i;
						for (i = 0; i < iwbClasses.length; i++) {
		...
		this.display = function(container) {
			...
			$('#' + container)
					.append(
							"<div id='flint-sidebar' style='width: " + $('#flint-editor').width()/2 + "px'>"
								...
								//	Removed temporarily until configurable sample queries are supported.
								//	+ "<li id='flint-sidebar-samples' style='white-space: wrap' title='View sample queries for the current dataset'>Samples</li>"
									+ "</ul><div id='flint-sidebar-content' style='overflow: auto'></div></div>"
...
//					}(tabsCount));
		...
		this.display = function(container) {
			$('#' + container)
					.append(
							"<div id='flint-editor-container'><div id='flint-editor-tabs-container'>"
//									+ "<div id='flint-scroll-tabs-left'><img src='"
//									+ flint.getImagesPath()
//									+ "/Previous.png'/></div>"
...
//									+ initialQuery + "</textarea>" 
			...
			$('#flint-editor-container').append($('#flint-code'));
			$('#flint-code').show();
...
//			}
			...
			this.showEndpointBar = function() {
				...
				var endpointUrl = endpointItem.getEndpoint();
				var item = config.endpoints[0]; 
				// endpointItem.getItem(endpointUrl);
