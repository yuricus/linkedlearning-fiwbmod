...
//       this.setSettings(chart, settings, this.StockSettingsMap);
       ...
       // PANEL ///////////////////////////////////////////       
        ...
        {
                dataSet.fieldMappings.push({fromField:"out"+ (y+1), toField:"out"+(y+1)});
                ...
                if(labels[y] != undefined)
                {
                    ...
                    if(eventsSettings != null)
                    	{
                    	if (!$.isArray(eventsSettings.event))
                    		eventsSettings.event = [ eventsSettings.event ];
                    	
	                    for(var m = 0; m < eventsSettings.event.length; m++)
	                    	{
	                    	  var event = eventsSettings.event[m];
	                    		if(event.graph != null && event.graph == labels[y])
	                    			dataSet.stockEvents.push(this.createEvent(graph, event));
	                    	}
                    	}
    ...
    createEvent:function(graph, event)
    {
    	var d;
    	if(event.date == "now")
    		d = new Date();
    	else
    		d =  this.parseDate(event.date);
    	
		var eventObject = {
				date: d,
				showOnAxis: AmCharts.toBoolean(event.showOnAxis),
				type: event.type,
    ...
    parseCSV:function(data, holder)
    {
        ...
        if(type == "column" || type == "line" || type == "radar" || type == "bar")
        {
            for (var i = 0; i < rows.length; i++)
            {           
                if(rows[i])
...
/* Load JS charts if possible. If the browser doesn't support SVG load Flash charts instead */
    ...
    function createFlashChart(chartContainerID, flashID, flashContent)
    {
             ...
             if(getDomElementById(chartContainerID))
             {
                 getDomElementById(chartContainerID).parentNode.innerHTML = flashContent;
                 if(getDomElementById(flashID+"_flash"))
                 {
                     getDomElementById(flashID+"_flash").style.visibility = 'visible';
                 }
     ...
     */
 AmCharts.replaceImages = function(anything) {
     /*
     ** PRIVATE FUNCTIONS
     */
   ...
   */
   ...
   for(var i1=0;i1==0;i1++) {
         ...
         var svgs = chartContainer[0].getElementsByTagName('svg');
        ...
        var heightCounter = 0;
         ...
         for(var i2=0;i2<svgs.length;i2++) {
             ...
             canvg(canvas,innerHTML,{ ignoreMouse: true, ignoreAnimation: true });
             
	         image.src = canvas.toDataURL();
	         image.setAttribute("class", "ImageForChart"); 
	           
	         wrapper.replaceChild(image, svgs[i2]);          
         }

         // Get the final data URL and throw throw the image to the array
