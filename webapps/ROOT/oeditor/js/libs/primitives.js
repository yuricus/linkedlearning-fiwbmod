...
function razeText(element, text, suffix, maxWidth, textProps, sufProps, tooltiper) {
    if (textProps.width + sufProps.width > maxWidth) {
        ...
        tooltiper.AddToolTip(element, function (d, i) {
            return text;
        });
    } else {
...
function addRectWithFullText(dataElement, width, vertMargin, horMargin, fill, text, textClass, parent, x, y, rx, ry) {
    ...
    a.render = function () {
        ...
        for (var index in listOFStrings) {
            ...
            var text = formD3ChainCalls(parent, "text#px" + index + "|id'px" + index + "|x'" + trueX + "|y'" + trueY + "|class'" + textClass);
            texts.push(text);
            text.text(curStr);
 ...
 * @param fromObject - из этого объекта берутся свойства
 */
...
function textInfo(stringOfText, textElementClass) {
    ...
    var xx = formD3ChainCalls(d3.select("body"),
        "div#" + textDifId + "|id'" + textDifId + "|style'position:absolute;visibility:hidden;white-space:nowrap;height:auto;width:auto;");
    if (arguments.length == 2) xx.attr('class', textElementClass);
    xx.text(stringOfText);
    //baseline
    var xx2 = formD3ChainCalls(d3.select("body"),
        "div#" + textDifId + "_2|id'" + textDifId + "_2|style'visibility:visible;position:absolute;vertical-align:baseline;");
