...
public class FURIInput extends FTextInput2 
{
   ...
   @Override
        ...
        switch ( event.getType() )
        {
	      //the list was clicked
	    	case MOUSE_LEFT:
	    	{
	    		...
	    		// populateView() triggers no input changed event ( getDomElementById('dropdown.id').value = "..." )
	    		populateView();
	    		populateValidity();
	    		break;
	    	}
            ...
            case BLUR: //set the value for the dropdown, UP/DOWN executes this
            {
                ...
                if(enablesuggestions)
                {
                    ...
                    URI uri = AutocompleteMap.getUriForName(_choices.get(currentSuggestions.get(idx)).getValue());
    ...
    public String render()
    {
		...
		if(!enablesuggestions){
			this.enablesuggestions = _choices != null && !getAndSetChoices().isEmpty();
		}
            ...
            if(suggList==null)
            {
                suggList = new FSuggestionList( "dropdown" ) 
                {
                    ...
                    public String render() 
                    {
                    	if (initialized) 
                    	{
                            List<AutocompletionOption<String>> choices = getAndSetChoices();
                            ...
                            if ( currentSuggestions!=null )
                                ...
                                {
                                    String displayString = choices.get(idx).getDisplayName();
                                    URI uri = AutocompleteMap.getUriForName(choices.get(idx).getValue());
                                    String s = null;
                                    if (uri != null)
                                    ...
                                    if (s == null) 
                                    {
                                    	 URI displayuri = AutocompleteMap.getUriForName(choices.get(idx).getValue());
                                         s = (displayuri != null) ? displayuri.stringValue() : null;
                                    }
                                    
                                    if (s != null)
                                        sb.append("<li onclick=\"catchEventId('"+FURIInput.this.getId()+"',1,'"+s+"');getDomElementById('"+FURIInput.this.getComponentid()+"').value='"+s+"';\" onmousedown=\"dropdownLiSelected();\">");
                                    FComponent display = choices.get(idx).getDisplay(this.value);
                                    if (display != null) 
                                    {
     ...
     */
    @Override
    public synchronized List<AutocompletionOption<String>> getChoices() 
    {
    	...
    	if (!StringUtil.isNullOrEmpty(this.query))
    	{
    		try
            {
                ...
                return convertArrayToOptionList(suggs);
            }
            catch (Exception e)
            {
                logger.warn("Invalid Query for FURIInput: " + e.getLocalizedMessage());
                return Collections.<AutocompletionOption<String>>emptyList();
            }
    	}
        ...
        if (predicate != null) 
        {
            ...
            if (autoCompleteMap!=null)
                return convertArrayToOptionList(autoCompleteMap);
            ...
            return convertArrayToOptionList(array.toArray(new String[0]));
        } 
        return null; 
    }
}
