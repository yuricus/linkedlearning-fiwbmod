...
public class IWBCmsUtil
{
	...
	/**
	 * Deletes metadata associated with the file. 
	 * In the upload method, all metadata associated with the file is written in the 
	 * context with the URI file.getURI().stringValue()+"_context". Thus, when deleting the file, the whole associated context is deleted
	 * with it. For backward compatibility, we also keep the legacy code which removes all statements with file.getURI() as a subject. 
	 * 
	 * @param file
	 * @param dm
	 */
	private static void deleteFileMetadataInternal(com.fluidops.iwb.cms.File file, ReadWriteDataManager dm) {
		dm.deleteContextById(getFileContextURI(file));
		// keep for backward compatibility for now.
		dm.removeInSpecifiedContext(new ReadWriteDataManagerImpl.NullableStatement(null, Vocabulary.SYSTEM.ATTACHEDFILE, file.getURI()), null);
        dm.removeInSpecifiedContext(new ReadWriteDataManagerImpl.NullableStatement(file.getURI(), null, null), null);
	}
	
	private static URI getFileContextURI(com.fluidops.iwb.cms.File file) {
		ValueFactory vf = ValueFactoryImpl.getInstance();
		return vf.createURI(file.getURI().stringValue()+"_context");
	}
}
