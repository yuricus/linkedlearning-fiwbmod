...
public class WikiManagementWidget extends AbstractWidget<WikiManagementWidget.Config>{
	...
	public static class Config extends WidgetBaseConfig
	{
		...
		@ParameterConfigDoc(desc = "list of visible filters", type=Type.LIST, required = false)
		public List<FilterDescription> visibleFilters;
		
		@ParameterConfigDoc(desc = "list of visible controls", type=Type.LIST, required = false)
	...
	protected static FButton createFilterTemplateButton(final WikiStorageBulkServiceImpl wsApi, final WikiPageSelectionTable wpTable) {
		return new FButton("b"+Rand.getIncrementalFluidUUID(), "Show template pages") {
			@Override
			public void onClick() {
				wpTable.updateTableModel( new TemplateFilter());
			} 			
		};
	}
	...
	protected static class WikiPageSelectionTable extends FSelectableTable<WikiPageMeta> {
		...
		private void initializeControlComponents() {	
			...
			if (isControlVisible(Control.RESTORELATESTREVISION)) addControlComponent(createRestoreLatestRevisionButton(wsApi, this, getDefaultFilter(config)), "floatLeft");
			...
			for (WikiPageMeta w : wsApi.getAllWikipages(filter)) {
				...
				tm.addRow(
						new Object[] {
								...
								new HtmlString(w.getRevision().comment),
								getTypeString(w),
	...
	protected static FButton createRestoreLatestRevisionButton(final WikiStorageBulkServiceImpl wsApi, final WikiPageSelectionTable wpTable, final Filter defaultFilter) {
		...
		return new FButton("restore", "Restore latest bootstrap") {
			...
			public void onClick() {
				...
				wpTable.updateTableModel(defaultFilter);
				FPopupWindow p = getPage().getPopupWindowInstance();
				p.removeAll();
				p.setTitle("Info");
	 ...
	 */
	private static Filter getFilterFor(FilterDescription filterDescription) {
	...
	/**
	 * Method for identifying the pre-defined filter from {@link Config#filter},
	 * if a pre-defined filter has been specified. We need this method for being
	 * able to access this information form any part of the code. Otherwise it
	 * is possible that the widget reloads applying a {@link VoidFilter}, thus
	 * displaying wiki pages it is not supposed to.
	 * 
	 * @param config
	 *            Config holding the pre-defined filter
	 * @return The filter specified in <code>config</code> or the
	 *         {@link VoidFilter} if none has been specified or it cannot be
	 *         resolved.
	 */
	private static Filter getDefaultFilter(Config config)
	{
