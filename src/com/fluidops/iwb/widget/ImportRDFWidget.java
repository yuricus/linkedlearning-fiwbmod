...
public class ImportRDFWidget extends AbstractWidget<WidgetVoidConfig>
{
	...
	{		
	    ...
	    refineContainer.add(createTitle("title6", "Format", 
	            "<p>The format of the source data in Open Refine</p>" ));
	    ...
	    tabpane.addTab("Open Refine", "imports data from a Open Refine Project. " +
	                "Open Refine should be running and the scheme for the data should be defined in the project", refineContainer);
			...
			public void onClick()
			{
				...
				if(fupload.filename!=null)
				{
					...
					if(!(dataTypeBox2.getSelected().get(0) instanceof RDFFormat)){
						...
						if(rf==null){
							getPage().getPopupWindowInstance().showError("The system was not able to detect the RDF format automatically. Please select the RDF Format manually!");
							return;
						}
					}else{
