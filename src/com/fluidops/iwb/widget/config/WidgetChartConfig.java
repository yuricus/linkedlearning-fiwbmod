...
public class WidgetChartConfig extends WidgetQueryConfig
{
    ...
    public WidgetEmbeddingError setOutputLabels(String id, List<String> outputs, ChartDataModel cdm){
    	...
    	//create a copy of the outputs to avoid manipulating the config parameter
    	List<String> outputLabels = new ArrayList<String>();

    	for(String output : outputs)
    	{
    		outputLabels.add(output);
    	}
    	
		if (variableConfiguration != null) {
			for (VariableConfig variableConfig : variableConfiguration) {
				...
				if (!Collections.replaceAll(outputLabels, variableConfig.variableName, variableConfig.displayName)) {
		...
		cdm.setLegendLabels(outputLabels.toArray(new String[outputLabels.size()]));
		cdm.setShowLegend(true);           
		cdm.setTitles(title, input, StringUtil.listToString(outputLabels, ", "));
