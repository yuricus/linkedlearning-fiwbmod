...
public class ComponentWidget extends AbstractWidget<ComponentWidget.Config> implements WidgetAccessControl
{
	...
	public FComponent getComponent(String id) 
	{
		try
		{
			...
			{
				FTable table = (FTable) c;

				if ( get().model != null )
					table.setModel( (TableModel) DynamicScriptingSupport.loadClass( get().model ).newInstance() );
				if ( get().rows != null )
					table.setNumberOfRows(get().rows);

				/*
				 * Enable filters for consistency with the TableResult widget
				 */
			...
			if ( c instanceof FTree )
			{
				...
				((FTree)c).setOverFlowContainer(true);
			}
			if ( c instanceof FEditTable )
			{
				...
				if ( get().rows != null )
					((FEditTable)c).table.setNumberOfRows(get().rows);
				((FEditTable)c).setOverFlowContainer(true);
			}
			if ( c instanceof FPage )
			{
				return new FPageComponent( id, (FPage)c, pc.getRequest() );
			}
