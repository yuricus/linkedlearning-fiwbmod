...
/**
 * The {@link RevisionTable} allows to perform actions on the revisions
		...
		FButton deleteSelected = new FButton(Rand.getIncrementalFluidUUID(), "Delete selected revisions") 
		{
			...
			{   
				if (subject != null) 
				{
					...
					// impl note: there is a slight trick here: we reverse the order of
					// deletion such we do not have to update the lucene index each time
					for(WikiRevisionWithRevNumber wrWithRev : Lists.reverse(revs)) {
