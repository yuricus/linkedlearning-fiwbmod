...
public class PrinterExtensionsImpl implements PrinterExtensions {
	...
	public String addToolbarButtons(PageContext pc, String htmlHead) 
	{
        ...
        if (pc.value!=null && pc.value instanceof URI && userManager.hasUIComponentAccess(UIComponent.HEADER_PRINT_BUTTON, null))
        {
        	...
        	String onClickAction = "var cont = $('.viewContentClazz').eq(0)[0].innerHTML; var w = window.open();  w.document.write(\'" + templatePref + "\' + cont + \'" + templateSuf + "\'); w.document.close();";
        ...
        // SPARQL Interface button
        if (userManager.hasUIComponentAccess(UIComponent.HEADER_SPARQL_BUTTON, null))
        {
			buttons += "<div class=\"topmenubarRightButtonSPARQL navButton\" onclick=\"$r('" + pc.contextPath + "/sparql');\"><img src=\"" + pc.contextPath + "/images/navigation/sparql.png\" /><div><span class=\"buttontext\">Query</span></div></div>";
			buttons += "<div class=\"topmenubarRightSpacer\"></div>";
        }
        ...
        if (UserContext.get() == null || !UserContext.get().hideLogout)
        {
		    ...
		    if (loggedIn)
		    {
				buttons += "<div class=\"topmenubarRightButtonLogout navButton\" onclick=\"$r('" + pc.contextPath + "/logout.jsp');\"><img src=\"" + pc.contextPath + "/images/navigation/logout.png\" /><div><span class=\"buttontext\">Logout</span></div></div>";
				buttons += "<div class=\"topmenubarRightSpacer\"></div>";
		    }
			else
			{
				...
				{
					String fullRequestURI = pc.getRequest().getRequestURI();
					if (pc.getRequest().getQueryString()!=null && !pc.getRequest().getQueryString().isEmpty())
						fullRequestURI += "?" + pc.getRequest().getQueryString();
